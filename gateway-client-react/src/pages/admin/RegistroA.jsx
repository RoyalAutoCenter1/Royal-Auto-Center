...
import { postEntityA, getAllEntityA } from '../../services/apiService';
...
const RegistroA = () => {
  ...
  const cargarDatos = async () => {
    try {
      const data = await getAllEntityA();
      setLista(data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await postEntityA({ nombreA: nombre.trim() }); // Enviamos el texto sin espacios extra a los lados
      setNombre('');
      cargarDatos();
    } catch (error) {
  ...
  return (
    <div className="p-4">
      <form onSubmit={handleSubmit} className="p-4 border rounded shadow-sm bg-white mb-4">
        <h3 className="mb-3">Registrar nueva Entity A</h3>
